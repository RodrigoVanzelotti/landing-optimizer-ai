...
def get_provider() -> LLMProvider:
    if settings.llm_provider == "openai" and settings.openai_api_key:
        logger.info("Provider selected", provider=settings.openai_model)
        return OpenAIProvider(
            api_key=settings.openai_api_key,
            base_url=settings.openai_base_url,
            model=settings.openai_model,
        )
    logger.info("Using stub provider", provider="stub-v1")
    return StubProvider()
...
def apply_guardrails(suggestion: Suggestion, guardrails: dict[str, Any]) -> Suggestion | None:
    ...
    if suggestion.proposed_value:
        ...
        if any(word in lowered for word in banned):
            logger.info("Dropping suggestion violating banned-words guardrail", reason="banned-word", banned_words=banned)
            return None
